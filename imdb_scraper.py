...
def read_html_table(element) -> list[list[str]]:
...
class Movie(Base):
    def __init__(self, movie_id, load_credits: bool = False):
...
def get_top_250_movies() -> list[Movie]:
    ...
    processed_table = process_table(read_html_table(table), first_index=1, second_index=2, save_href=True)
    for movie_id, rating in processed_table:
        movies.append(Movie(movie_id))
        movies[-1].rating = rating
    return movies
